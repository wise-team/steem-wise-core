...
// wise imports
import { Wise, SteemOperationNumber, Api, SingleDaemon } from "../wise";
...
describe("test/unit/synchronization-retrospective.spec.ts", () => {
    ...
    describe("Synchronizer - retrospective test (based on real blockchain data)", function() {
        ...
        it("Starts synchronization without error", () => {
            const synchronizationPromiseReturner = async () =>
                await new BluebirdPromise<void>((resolve, reject) => {
                    synchronizer = delegatorWise.startDaemon(
                        new SteemOperationNumber(fromBlock, 0, 0),
        ...
        it("Confirmations match cases", () => {
            cases.forEach(case_ => {
                ...
                expect(matching[0].accepted, "voteorder " + matching[0].voteorderTx + " accepted").to.be.equal(
                    case_.shouldAccept
                );
            });
        });
