...
describe("test/unit/synchronization.spec.ts", () => {
    describe("Synchronizer", function() {
        ...
        it("does nothing on voteorder sent before rules", async () => {
            const post: steem.SteemPost = Util.definedOrThrow(
                _.sample(fakeDataset.posts),
                new Error("post is undefined")
            );
        ...
        it("Delegator sets rules for voter", async () => {
            const sendRulesets: Ruleset[] = [
                {
                    ...
                    rules: [new AuthorsRule(AuthorsRule.Mode.ALLOW, ["noisy"]), new WeightRule(0, 2000)],
                },
                {
                    name: "RulesetTwoWillBeRemoved",
                    rules: [new AuthorsRule(AuthorsRule.Mode.ALLOW, ["perduta"]), new WeightRule(0, 2000)],
                },
            ];
            ...
            expect(gotRulesets)
                .to.be.an("array")
                .with.length(sendRulesets.length);
        ...
        validVoteorders1.forEach((voteorder: SendVoteorder) =>
            it(
                "Voter sends valid voteorder (ruleset=" + voteorder.rulesetName + ") and delegator passes them",
                async () => {
                    ...
                    const handledOps: EffectuatedWiseOperation[] = Util.definedOrThrow(
                        delegatorWise.getProtocol().handleOrReject(lastPushedTrx)
                    );
        ...
        invalidVoteorders1.forEach((voteorder: SendVoteorder) =>
            it(
                "Voter sends invalid voteorder (ruleset=" + voteorder.rulesetName + ") and delegator rejects them",
                async () => {
                    const skipValidation = true;
                    ...
                    const lastHandledOp = Util.definedOrThrow(_.last(handledOps));
                    expect(ConfirmVote.isConfirmVote(lastHandledOp.command), "last pushed op is confirm vote").to.be
                        .true;
                    expect((lastHandledOp.command as ConfirmVote).accepted, "last pushed op is accepted confirm vote")
                        .to.be.false;
                }
            )
        );
        ...
        previouslyValidNowInvalidVoteorders.forEach((voteorder: SendVoteorder) =>
            it(
                "Voter sends previously valid (but now invalid) voteorder (ruleset= " +
                    voteorder.rulesetName +
                    ") and delegator rejects them",
                async () => {
                    const skipValidation = true;
                    const moment = await voterWise.sendVoteorder(delegator, voteorder, () => {}, skipValidation);
                    expect(moment.blockNum).to.be.greaterThan(0);
        ...
        previouslyInalidNowValidVoteorders.forEach((voteorder: SendVoteorder) =>
            it(
                "Voter sends now valid (but previously invalid) voteorder (rulesetName= " +
                    voteorder.rulesetName +
                    ") and delegator passes them",
                async () => {
                    const moment = await voterWise.sendVoteorder(delegator, voteorder);
                    expect(moment.blockNum).to.be.greaterThan(0);
                    await BluebirdPromise.delay(100);
        ...
        it("Delegator rejects nonexistent post with negative confirmation but without error", async () => {
            ...
            try {
                ...
                if ((e as ValidationException).validationException)
                    throw new Error("Should not throw ValidationException, but pass it");
                else throw e;
            }
        });
