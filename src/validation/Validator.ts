...
export class Validator {
    ...
    public provideRulesets(rulesets: EffectuatedSetRules) {
        Log.log().debugGen(() => ["VALIDATOR_PROVIDED_WITH_RULESETS=" + JSON.stringify(rulesets)]);
    ...
    public async validate(
        ...
        Log.log().debugGen(() => [
            "VALIDATOR_VALIDATE=" +
                JSON.stringify({ delegator: delegator, voter: voter, voteorder: voteorder, atMoment: atMoment }),
        ]);
        try {
            ...
            Log.log().debugGen(() => ["VALIDATOR_USING_RULESETS=" + JSON.stringify(rulesets)]);
    ...
    private validateVoteorderObject(voteorder: SendVoteorder) {
        ...
        if (typeof voteorder.rulesetName === "undefined" || voteorder.rulesetName.length == 0)
            throw new ValidationException("Ruleset_name must not be empty");
        if (typeof voteorder.author === "undefined" || voteorder.author.length == 0)
            throw new ValidationException("Author must not be empty");
        if (typeof voteorder.permlink === "undefined" || voteorder.permlink.length == 0)
            throw new ValidationException("Permlink must not be empty");
        if (typeof voteorder.weight === "undefined" || isNaN(voteorder.weight))
            throw new ValidationException("Weight must not be empty");
    ...
    private selectRuleset = (rulesets: SetRules, voteorder: SendVoteorder): Rule[] => {
        for (let i = 0; i < rulesets.rulesets.length; i++) {
            ...
            if (ruleset.name === voteorder.rulesetName) {
                Log.log().debugGen(() => ["VALIDATOR_SELECTED_RULESET=" + JSON.stringify(ruleset.rules)]);
                return ruleset.rules;
            }
        }
        throw new ValidationException(
            "Delegator had no such ruleset (name=" + voteorder.rulesetName + ") at specified datetime."
        );
    };

    private async validateRules(rules: Rule[], voteorder: SendVoteorder, context: ValidationContext): Promise<void> {
        await BluebirdPromise.resolve(rules).map(
            async rule => {
                try {
                    await rule.validate(voteorder, context);
                    Log.log().debugGen(() => ["VALIDATOR_RULE_VALIDATION_SUCCEEDED=" + JSON.stringify(rule)]);
                } catch (error) {
                    Log.log().debugGen(() => [
                        "VALIDATOR_RULE_VALIDATION_FAILED=" + JSON.stringify({ rule: rule, error: error.message }),
                    ]);
                    throw error;
                }
            },
