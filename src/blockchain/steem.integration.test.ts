...
describe("test/integration/steem.spec.ts", () => {
    ...
    describe("account_history_api.get_account_history", function() {
        ...
        it("Returns incorrect op_in_trx", async () => {
            const result: steemJs.AccountHistory.Operation[] = await steem.getAccountHistoryAsync(
                "steemprojects1",
                40,
                0
            );
            if (result.length == 0) {
                ...
                if (rawOp[1].op_in_trx === 3)
                    throw new Error(
                        "Steem account_history_api returns correct op_in_trx (" +
                            rawOp[1].op_in_trx +
                            "). The bug was repaird. Please replace all lesserThan_solveOpInTrxBug" +
                            " to lesserThan."
                    );
                else {
                    Log.log().warn(
                        "Steem account_history_api returns wrong op_in_trx (" +
                            rawOp[1].op_in_trx +
