...
describe("RulesUpdater", function() {
    const rules0: SetRulesForVoter[] = [
        {
            ...
            rulesets: [
                {
                    name: "a",
                    rules: [new WeightRule(0, 100), new TagsRule(TagsRule.Mode.REQUIRE, ["steemprojects"])],
                },
            ],
        },
        {
            ...
            rulesets: [
                {
                    name: "b",
                    rules: [new AuthorsRule(AuthorsRule.Mode.ALLOW, ["noisy"])],
                },
            ],
        },
    ];
    ...
    it("Wise.downloadRules downloads rules correctly", () => {
        return delegatorWise.downloadAllRulesets().then((result: EffectuatedSetRules[]) => {
            const difference = _.differenceWith(
                result,
                rules0,
                (itemA, itemB) => _.isEqual(itemA.rulesets, itemB.rulesets) && _.isEqual(itemA.voter, itemB.voter)
            );
            expect(difference, "difference between downloaded & set rules")
                .to.be.an("array")
                .with.length(0);
        });
    });
    ...
    it("Wise.downloadRules downloads rules correctly", () => {
        return delegatorWise.downloadAllRulesets().then((result: EffectuatedSetRules[]) => {
            const difference = _.differenceWith(
                result,
                rules8,
                (itemA, itemB: EffectuatedSetRules) =>
                    _.isEqual(itemA.rulesets, itemB.rulesets) && _.isEqual(itemA.voter, itemB.voter)
            );
            expect(difference, "difference between downloaded & set rules")
                .to.be.an("array")
                .with.length(0);
