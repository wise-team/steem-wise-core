        ...
        describe("api " + api.name(), () => {
            ...
            describe("#loadPost", () => {
                it("loads correct post", async () => {
                    const post: steem.SteemPost = await api.loadPost(
                        "noisy",
                    ...
                    expect(post.permlink).to.be.equal(
                        "dear-whales-please-consider-declining-all-comment-rewards-by-default-in-settings-5-reasons-to-do-that"
                    );
                    assert(
                        post.body.indexOf(
                            "Declining a payout from a comment was possible even earlier, however it was always difficult to do that without some programming skills."
                        ) !== -1
                    );
            ...
            describe("#loadRulesets (for delegator and voter)", () => {
                ...
                it("loads proper rules (v1)", async () => {
                    ...
                    expect(r.rulesets, "rulesets")
                        .to.be.an("array")
                        .with.length(v1TestingSequence.stage1_0_Rulesets.rulesets.length);
            ...
            describe("#sendToBlockchain", () => {
                it("Sends without error and returns non-zero SteemOperationNumber", async () => {
                    const son = await api.sendToBlockchain([
                        [
                            "vote",
                            {
                                voter: "guest123",
            ...
            describe("#loadRulesets (for delegator and all voters)", () => {
                it("Loads properly all rulesets from protocol-v1-testing-sequence", async () => {
                    ...
                    const result: EffectuatedSetRules[] = await api.loadRulesets(
                        { delegator: v1TestingSequence.delegator },
                        v1TestingSequence.stage3_1_SyncConfirmationMoment
                    );
                ...
                it("Does load properly rulesets that were current for noisy at b23589679", async () => {
                    const result: EffectuatedSetRules[] = await api.loadRulesets(
                        { delegator: "noisy" },
                        new SteemOperationNumber(23589679, 0, 0)
                    );
                    result.forEach(esr => {
                        if (esr.voter === "innuendo") {
                            // returns only tyhe most current rulesey for innuendo
                ...
                it("Does not load any rulesets if user did not have any at the moment", async () => {
                    const result: EffectuatedSetRules[] = await api.loadRulesets(
                        { delegator: "innuendo" },
                        new SteemOperationNumber(24389679, 0, 0)
                    );
                    expect(result)
                        .to.be.an("array")
                        .with.length(0);
                ...
                it("Loads all new rulesets when moment is SteemOperationNumber.FUTURE and does not load the old ones", async () => {
                    ...
                    result.forEach(esr => {
                        if (esr.voter === "aaa") {
                            expect(esr.moment.blockNum).to.be.equal(29510066);
                        }
                        expect(esr.moment.blockNum).to.be.gte(27496789);
                    });
                });
            });
            ...
            describe("#getAllWiseOperationsInBlock", () => {
                ...
                it("Waits for future block", async function() {
                    ...
                    for (const api of apis)
                        if (api.name() === "FakeApi")
                            _.times(2, num => setTimeout(() => ((api as any) as FakeApi).pushFakeBlock(), 200 * num));
                ...
                it("returns ConfirmVoteBoundWithVote instead of pure ConfirmVote", async () => {
                    ...
                    expect(ops)
                        ...
                        .with.length(1);
                    expect(
                        ConfirmVoteBoundWithVote.isConfirmVoteBoundWithVote(ops[0].command),
            ...
            describe("#getWiseOperationsRelatedToDelegatorInBlock", () => {
                it("Loads only wise operation from single block", async () => {
                    const blockNum = 23487915;
                    const ops: EffectuatedWiseOperation[] = await api.getWiseOperationsRelatedToDelegatorInBlock(
                        "steemprojects3",
                        blockNum
                    );
                    expect(ops)
            ...
            describe("#getDynamicGlobalProperties", () => {
                it("returns dynamic global properties without error", async () => {
                    ...
                    expect(new Date(dgp.time + "Z").getTime(), "time").to.be.closeTo(
                        new Date().getTime(),
                        1000 * 10 /* 10 seconds */
                    );
                });
            ...
            describe("#getAccountInfo", () => {
                it("returns account info without error", async () => {
                    ...
                    expect(info.delegated_vesting_shares, "delegated_vesting_shares").to.match(
                        /^([0-9]+)\.([0-9]+)\ VESTS$/
                    );
                    ...
                    expect(info.received_vesting_shares, "received_vesting_shares").to.match(
                        /^([0-9]+)\.([0-9]+)\ VESTS$/
                    );
                    ...
                    expect(new Date(info.last_vote_time + "Z").getTime(), "last_vote_time").to.be.gte(
                        new Date("2018-05-30T12:25:36").getTime()
                    );
                });
            ...
            describe("#getWiseOperations", () => {
                it("Loads only wise operation that are newer than until", async () => {
                    ...
                    expect(ops[0].moment.transactionNum, "ops[0] transaction_num")
                        .to.be.a("number")
                        .that.is.gte(0);
                });
                ...
                it("Returns in order from newest to oldest", async () => {
                    ...
                    entries.forEach((entry: steem.BlogEntry) => {
                        ...
                        if (currentEntryTime > 0) {
                            // this time is only > 0 for reblogs, but is useful for asserting order
