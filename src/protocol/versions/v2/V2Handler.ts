...
export class V2Handler implements ProtocolVersionHandler {
    ...
    private decode = (
        transaction: UnifiedSteemTransaction,
        wiseOp: wise_operation,
        sender: string
    ): EffectuatedWiseOperation[] | undefined => {
    ...
    private decodeSetRules = (
        transaction: UnifiedSteemTransaction,
        wiseOp: wise_set_rules_operation,
        sender: string
        ...
        const out: EffectuatedWiseOperation = {
            moment: new SteemOperationNumber(
                transaction.block_num,
                transaction.transaction_num,
                0 /* skip operation num due to rejection that was done in #handleOrReject */
            ),
    ...
    private decodeSendVoteorder = (
        transaction: UnifiedSteemTransaction,
        wiseOp: wise_send_voteorder_operation,
        sender: string
    ): EffectuatedWiseOperation[] => {
    ...
    private decodeConfirmVote = (
        transaction: UnifiedSteemTransaction,
        wiseOp: wise_confirm_vote_operation,
        sender: string
    ): EffectuatedWiseOperation[] => {
    ...
    public serializeToBlockchain = (op: WiseOperation): steem.OperationWithDescriptor[] => {
        ...
        const completeOp: steem.OperationWithDescriptor = [
            "custom_json",
            {
