...
// wise imports
import { ExpirationDateRule, SendVoteorder, ValidationException, Wise } from "../wise";
...
describe("test/unit/rule-expirationdate.spec.ts", () => {
    describe("ExpirationDateRule", function() {
        const tests: { name: string; ruleDate: () => string; pass: boolean }[] = [
            ...
            {
                name: "Passes non expired rule",
                ruleDate: () => new Date(Date.now() + 60 * 1000).toISOString(),
                pass: true,
            },
            {
                name: "Fails on expired rule",
                ruleDate: () => new Date(Date.now() - 60 * 1000).toISOString(),
                pass: false,
            },
        ];
        ...
        tests.forEach((test, i: number) =>
            it("ExpirationDateRule: " + test.name, () => {
