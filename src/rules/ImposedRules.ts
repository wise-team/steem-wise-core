...
import { BlockchainConfig } from "steem-efficient-stream";
...
export namespace ImposedRules {
    export class NoDustRule extends Rule {
        ...
        public async validate(voteorder: SendVoteorder, context: ValidationContext) {
            ...
            const voter_effective_vesting_shares =
                this.vestsToFloat(accountInfo.vesting_shares) -
            ...
            const regenerated_power =
                (BlockchainConfig.STEEM_100_PERCENT * elapsed_seconds) /
                BlockchainConfig.STEEM_VOTING_MANA_REGENERATION_SECONDS;
            const current_power = Math.min(
                accountInfo.voting_power + regenerated_power,
                BlockchainConfig.STEEM_100_PERCENT
            );
            if (current_power <= 0)
                throw new ValidationException("Account " + this.voter + " currently does not have voting power.");
            ...
            const max_vote_denom =
                votes_per_regeneration_period * BlockchainConfig.STEEM_VOTING_MANA_REGENERATION_SECONDS;
            if (max_vote_denom <= 0)
                throw new ValidationException("Account " + this.voter + ": max_vote_denom = " + max_vote_denom);

            used_power = (used_power + max_vote_denom - 1) / max_vote_denom;
            if (max_vote_denom <= 0)
                throw new ValidationException("Account " + this.voter + " does not have enough power to vote.");
            ...
            if (abs_rshares <= BlockchainConfig.STEEM_VOTE_DUST_THRESHOLD)
        ...
        public type(): Rule.Type {
            throw new Error(
                "This is and imposed rule. It cannot be added to a SetRules ruleset. It is only intended to be used during a voteorder validation"
            );
        }
