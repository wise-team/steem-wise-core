...
// wise imports
import { VotingPowerRule, SendVoteorder, ValidationException } from "../wise";
...
describe.only("VotingPowerRule", function() {
    describe("#validateRuleObject", () => {
        it("does not allow voting power greater than 100% (10000)", () => {
            const rule = new VotingPowerRule(VotingPowerRule.Mode.EQUAL, 10001);
            expect(() => rule.validateRuleObject(rule))
                .to.throw()
                .that.haveOwnProperty("message")
                .that.contains("greater")
                .and.contains("100%");
        });

        it("does not allow voting power less than 0% (0)", () => {
            const rule = new VotingPowerRule(VotingPowerRule.Mode.EQUAL, -1);
            expect(() => rule.validateRuleObject(rule))
                .to.throw()
                .that.haveOwnProperty("message")
                .that.contains("less")
