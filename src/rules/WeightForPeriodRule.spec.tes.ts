...
import {
    ...
    wise_rule_weight_for_period_encode,
    wise_rule_weight_for_period_decode,
} from "../protocol/versions/v2/rules/rule-weight-for-period-schema";
import { ConfirmVote } from "../protocol/ConfirmVote";
import { ConfirmVoteBoundWithVote } from "../protocol/ConfirmVoteBoundWithVote";
import { SynchronizerTestToolkit } from "../synchronizer/_test/SynchronizerTestToolkit";
...
describe("test/unit/rule-weightforperiod.spec.ts", () => {
    describe("WeightForPeriodRule", () => {
        const testsPerPeriod: {
            name: string;
            voteorders: { deltaTime: number; weight: number; voter: string }[];
            period: number;
            ruleWeight: number;
            nextVoteorderWeight: number;
            voter: string;
            pass: boolean;
        }[] = [
            {
                name: "passes when no votes over period 10",
                pass: true,
                voteorders: [],
                voter: voterA,
                period: 10,
                ruleWeight: 100,
                nextVoteorderWeight: 1,
            },
            {
                name: "fails when too much vote weight in single vote in period",
                voter: voterA,
                pass: false,
            ...
            {
                name: "passes when correct vote weight in single vote in period",
                voter: voterA,
                pass: true,
            ...
            {
                name: "passes when too much vote weight in single vote outside of period",
                voter: voterA,
                pass: true,
            ...
            {
                name: "fails when too much vote weight in two votes in period",
                voter: voterA,
                pass: false,
                voteorders: [
                    { deltaTime: -8, weight: 50, voter: voterA },
                    { deltaTime: -2, weight: 51, voter: voterA },
                ],
                period: 10,
                ruleWeight: 100,
                nextVoteorderWeight: 1,
            ...
            {
                name: "passes when too much vote weight in two votes outside period",
                voter: voterA,
                pass: true,
                voteorders: [
                    { deltaTime: -12, weight: 50, voter: voterA },
                    { deltaTime: -2, weight: 51, voter: voterA },
                ],
                period: 10,
                ruleWeight: 100,
            ...
            {
                name: "passes when too much vote weight in two votes outside period and one vote is a flag ",
                voter: voterA,
                pass: true,
                voteorders: [
                    { deltaTime: -12, weight: -50, voter: voterA },
            ...
            {
                name: "fails when too much vote weight in two votes + new voteorder",
                voter: voterA,
                pass: false,
                voteorders: [
                    { deltaTime: -7, weight: 50, voter: voterA },
                    { deltaTime: -2, weight: 50, voter: voterA },
                ],
                period: 10,
                ruleWeight: 100,
                nextVoteorderWeight: 1,
            ...
            {
                name: "passes when too much vote weight in two votes, but the second vote is from different voter",
                voter: voterA,
                pass: true,
                voteorders: [
                    { deltaTime: -7, weight: 50, voter: voterA },
                    { deltaTime: -2, weight: 50, voter: voterB },
                ],
                period: 10,
                ruleWeight: 100,
                nextVoteorderWeight: 1,
        ...
        const tests: {
            ...
            voteorders: { deltaTime: number; weight: number; voter: string }[];
            period: number;
            unit: WeightForPeriodRule.PeriodUnit;
            ruleWeight: number;
            nextVoteorderWeight: number;
            voter: string;
            pass: boolean;
        ...
        units.forEach(unit => {
            testsPerPeriod.forEach(t => {
                tests.push({
                    name: t.name,
                    voter: t.voter,
                    voteorders: t.voteorders.map(vo => ({
                        deltaTime: vo.deltaTime * unit.multiplier,
                        weight: vo.weight,
                        voter: vo.voter,
                    })),
                    period: t.period,
                    unit: unit.unit,
                    ruleWeight: t.ruleWeight,
                    nextVoteorderWeight: t.nextVoteorderWeight,
                    pass: t.pass,
                });
            });
        });
        ...
        tests.forEach((test, index) =>
            describe(test.name + ", unit: " + test.unit, () => {
                let fakeApi: Api;
                ...
                it("Sets rules", () => {
                    const rulesets: Ruleset[] = [{ name: "ruleset", rules: [] }];
                    const voters: string[] = _.uniq(test.voteorders.map(vo => vo.voter));
                    // set rules for each voter in the voteorders
                    return BluebirdPromise.resolve(voters)
                        .mapSeries((voter: any /* 'any' because of a bug in BluebirdPromise */) =>
                ...
                test.voteorders.forEach((vo: { deltaTime: number; weight: number; voter: string }) => {
                    ...
                    it("Sends voteorder at time " + fakeTime, () => {
                        ...
                        return voterWise
                            .generateVoteorderOperations(delegator, vo.voter, voteorder)
                            .then((ops: steem.OperationWithDescriptor[]) =>
                                ((fakeApi as any) as FakeApi).sendToBlockchain(ops)
                            )
                ...
                it("ValidationContext.getWiseOperations(voter, until=50 days) returns correct number of voteorders", () => {
                    ...
                    return BluebirdPromise.resolve(voters)
                        .mapSeries((voter: any /* 'any' because of a bug in BluebirdPromise */) =>
                            context
                                .getWiseOperations(
                                    voter /*! voter as we are testing sending, not synchronization !*/,
                                    until
                                )
                                .then((ops: EffectuatedWiseOperation[]) => {
                                    ops.forEach(op => {
                                        expect(op.timestamp.getTime(), "operation timestamp").to.be.greaterThan(
                                            until.getTime(),
                                            "until"
                                        );
                                    });
                ...
                it("ValidationContext.getWiseOperations(delegator, until=50 days) returns correct number of confirmations, all bound with vote operation", () => {
                    ...
                    return context
                        .getWiseOperations(context.getDelegatorUsername(), until)
                        .then((ops: EffectuatedWiseOperation[]) => {
                            expect(ops.filter(op => ConfirmVote.isConfirmVote(op.command)))
                                .to.be.an("array")
                                .with.length(test.voteorders.length);
                ...
                it("Validates rule: " + test.name + ", unit: " + test.unit, () => {
                    ...
                    return rule.validate(voteorder, context, nowTime).then(
                        () => {
